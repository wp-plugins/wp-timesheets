<div class="wrap">
	<h2><?=sprintf( __( '%s Timedata' , 'wp_timesheets' ), ucfirst($verb) )?></h2>
...
<?php if ( $show_description == 1 || !empty($description) ) : ?>		
			<tr>
				...
				<td><input name="description" id="description" class="regular-text" value="<?=$description?>"/><p class="description"><?=_e('In a few words, explain about the job done.', 'wp_timesheets' )?></p></fieldset></td>
			</tr>
<?php endif; ?>			
<?=WP_Timesheets_Admin::display_other_fields($other_field)?>
		</table>
		<p class="submit">
			<input type="submit" name="action" id="action" class="button button-primary" value="<?=_e('Save', 'wp_timesheets' )?>">
			<a class="button button-secondary" href="?page=wp_timesheets_user_report">Cancel</a>
		</p>
	</form>
